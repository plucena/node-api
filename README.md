...
EVM nodes speak JSON-RPC over HTTP and WebSocket, so what remains is the definition of which JSON-RPC calls to make, and against which node.
...
**Escaping the shared endpoint.** Take the QuickNode pattern: a dedicated endpoint from a managed provider, meaning your own keyed URL with your own quota, WebSocket push and webhook-style streams, with none of the operating work.

Cheapest of all, before that: give the client a viem `fallback()` transport across every endpoint you have, so one operator's bad minute isn't an outage.
